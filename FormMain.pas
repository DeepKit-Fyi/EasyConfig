...
uses
  ...
  Vcl.ExtCtrls, System.Actions, Vcl.ActnList, System.JSON, JSONHelpers, System.IOUtils,
  ControllerConfigs, ConfigManager, UtilsTypes, ErrorLogger, ErrorDialog, ExceptionRecovery,
  LazyLoadManager, OptimizedJSONTreeView, OptimizedINIGrid, MemoryOptimizer;
...
type
  TFrmMain = class(TForm)
    ...
    procedure tmrMemoryCheckTimer(Sender: TObject);
  private
...
begin
  ...
  // 初始化内存优化器
  MemOptimizer.Initialize;
  MemOptimizer.AutoOptimizeThreshold := 200 * 1024 * 1024; // 200MB
  MemOptimizer.OptimizeInterval := 120; // 2分钟
  MemOptimizer.MaxCacheSize := 200; // 最多缓存200个JSON对象

  // 初始化延迟加载管理器
  LazyLoader.MaxBatchSize := 20; // 每批加载20个节点
  LazyLoader.BatchDelay := 30; // 批次间隔30毫秒

  // 初始化内存检查定时器
  tmrMemoryCheck := TTimer.Create(Self);
  tmrMemoryCheck.Interval := 30000; // 30秒
  tmrMemoryCheck.OnTimer := tmrMemoryCheckTimer;
  tmrMemoryCheck.Enabled := True;
...
procedure TFrmMain.tmrMemoryCheckTimer(Sender: TObject);
begin
  // 检查并优化内存
  MemOptimizer.CheckAndOptimize;
end;
