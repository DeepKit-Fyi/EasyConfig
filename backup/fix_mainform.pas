// 浠ヤ笅鏄渶瑕佹坊鍔犲埌MainForm.pas鏂囦欢鐨勬柟娉曞疄鐜?
...
begin
  ...
  begin
    ...
    begin
      ...
      if P = 0 then
      begin
        // 褰撴病鏈夌偣鍙锋椂锛屾煡鎵捐琛屾槸鍚﹀湪鍊煎垪琛ㄧ紪杈戝櫒涓湁[Section]杩欐牱鐨勬牸寮忚〃绀鸿妭
        if (Value.StartsWith('[') and Value.EndsWith(']')) then
        begin
          IsSection := True;
          SectionName := Copy(Value, 2, Length(Value) - 2); // 鎻愬彇鑺傚悕锛堝幓鎺夋柟鎷彿锛?
          
          // 缁樺埗鑺傛爣棰樿
          Grid.Canvas.Brush.Color := $00E6FFEE; // 娴呯豢鑹茶儗鏅?
          Grid.Canvas.Font.Style := [fsBold];
          ...
          // 濡傛灉鏄浜屽垪锛屾竻绌鸿鍒楋紙鑺傛爣棰樿涓嶉渶瑕佸€硷級
          if ACol = 1 then
          begin
    ...
    begin
      ...
      begin
        // 涓轰氦鏇胯搴旂敤涓嶅悓鐨勮儗鏅鑹?
        if Odd(ARow) then
          Grid.Canvas.Brush.Color := $00F5F5F5 // 娴呯伆鑹?
        else
...
begin
  if InputQuery('娣诲姞鏂拌妭', '璇疯緭鍏ユ柊鑺傚悕绉?', NewSectionName) then
  begin
    ...
    begin
      try
        ...
        FormHelper.AddDebugText(meoDebug, '娣诲姞鏂拌妭: ' + NewSectionName);
        
        // 璁剧疆涓哄凡淇敼鐘舵€?
        FBasicInfoTab.Tag := 1; // 琛ㄧず鏈夋湭淇濆瓨鐨勬洿鏀?
      except
        on E: Exception do
        begin
          FormHelper.AddDebugText(meoDebug, '娣诲姞鏂拌妭鏃跺嚭閿? ' + E.Message);
          ShowMessage('娣诲姞鏂拌妭澶辫触: ' + E.Message);
        end;
      end;
    end;
  end;
